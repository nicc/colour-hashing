...
function assertPromise(value, message) {
  assert2(value instanceof Promise, message ?? "Expected a promise");
  return value;
}
...
function fastInverse(x, p3, n, kmax, twoToMinusKmax) {
  ...
  s = mod(s << kmax - k, p3);
  assert2(mod(x * s - 1n, p3) === 0n, "mod p");
  return s;
}
function sqrt(n_, p3, Q, c, M) {
  let n = mod(n_, p3);
  if (n === 0n)
    return 0n;
...
function createField(p3, constants) {
  ...
  return {
    ...
    equal(x, y) {
      let x_ = x >= 0n && x < p3 ? x : mod(x, p3);
      let y_ = y >= 0n && y < p3 ? y : mod(y, p3);
      return x_ === y_;
    },
    isEven(x) {
      return !(mod(x, p3) & 1n);
...
function Endomorphism(Field5, Scalar4, generator, a2, endoScalar, endoBase) {
  if (endoScalar === void 0 || endoBase === void 0) {
    try {
      ({ endoScalar, endoBase } = computeEndoConstants(Field5, Scalar4, generator, a2));
    } catch (e) {
      return void 0;
    }
  }
  ...
  return {
    ...
    scaleProjective(g, s) {
      return glvScaleProjective(g, s, Field5.modulus, a2, endoBase_, glvData);
    },
    scale(g, s) {
      let gProj = projectiveFromAffine(g);
      let sGProj = glvScaleProjective(gProj, s, Field5.modulus, a2, endoBase_, glvData);
...
function glvScaleProjective(g, s, p3, a2, endoBase, data) {
  let endoG = endomorphismProjective(g, endoBase, p3);
  let double2 = getProjectiveDouble(p3, a2);
  ...
  for (let i = data.maxBits - 1; i >= 0; i--) {
    if (S0[i])
      h = projectiveAdd(h, g, p3, a2);
    if (S1[i])
      h = projectiveAdd(h, endoG, p3, a2);
    if (i === 0)
      break;
    h = double2(h, p3);
  }
...
function computeEndoConstants(Field5, Scalar4, G2, a2) {
  let p3 = Field5.modulus;
  let q3 = Scalar4.modulus;
  assert(p3 % 3n === 1n, "Base field has a cube root of unity");
  assert(q3 % 3n === 1n, "Scalar field has a cube root of unity");
  let sqrtMinus3 = Scalar4.sqrt(Scalar4.negate(3n));
  assert(sqrtMinus3 !== void 0, "Scalar field has a square root of -3");
  let lambda = Scalar4.div(Scalar4.sub(sqrtMinus3, 1n), 2n);
  assert(lambda !== void 0, "Scalar field has a cube root of unity");
  assert(Scalar4.power(lambda, 3n) === 1n, "lambda is a cube root");
  assert(lambda !== 1n, "lambda is not 1");
  let lambdaG = affineScale(G2, lambda, p3, a2);
  assert(lambdaG.y === G2.y, "multiplication by lambda is a cheap endomorphism");
  let beta = Field5.div(lambdaG.x, G2.x);
  assert(beta !== void 0, "Gx is invertible");
  assert(Field5.power(beta, 3n) === 1n, "beta is a cube root");
  assert(beta !== 1n, "beta is not 1");
  let R = affineScale(G2, Scalar4.random(), p3, a2);
  let lambdaR = affineScale(R, lambda, p3, a2);
  assert(lambdaR.x === Field5.mul(beta, R.x), "confirm endomorphism");
...
function projectiveAdd(g, h, p3, a2) {
  if (g.z === 0n)
    return h;
  if (h.z === 0n)
    return g;
  ...
  if (H === 0n) {
    if (S1 === S2)
      return projectiveDouble(g, p3, a2);
    if (mod(S1 + S2, p3) === 0n)
...
function projectiveDoubleA0(g, p3) {
  if (g.z === 0n)
    return g;
...
function projectiveDoubleAminus3(g, p3) {
  if (g.z === 0n)
    return g;
  let X1 = g.x, Y1 = g.y, Z1 = g.z;
  if (Y1 === 0n)
    throw Error("projectiveDouble: unhandled case");
  let delta = mod(Z1 * Z1, p3);
  let gamma = mod(Y1 * Y1, p3);
  let beta = mod(X1 * gamma, p3);
  let alpha = mod((X1 - delta) * (X1 + delta), p3);
  alpha = alpha + alpha + alpha;
  let X3 = mod(alpha * alpha - 8n * beta, p3);
  let Z3 = mod((Y1 + Z1) * (Y1 + Z1) - gamma - delta, p3);
  let Y3 = mod(alpha * (4n * beta - X3) - 8n * gamma * gamma, p3);
  return { x: X3, y: Y3, z: Z3 };
}
function projectiveDouble(g, p3, a2) {
  if (a2 === 0n)
    return projectiveDoubleA0(g, p3);
  if (a2 + 3n === p3)
    return projectiveDoubleAminus3(g, p3);
  throw Error("Projective doubling is not implemented for general curve parameter a, only a = 0 and a = -3");
}
function getProjectiveDouble(p3, a2) {
  if (a2 === 0n)
...
function projectiveSub(g, h, p3, a2) {
  return projectiveAdd(g, projectiveNeg(h, p3), p3, a2);
}
function projectiveScale(g, x, p3, a2) {
  let double2 = getProjectiveDouble(p3, a2);
...
function projectiveOnCurve({ x, y, z }, p3, b2, a2) {
  let x3 = mod(mod(x * x, p3) * x, p3);
  let y2 = mod(y * y, p3);
  let z2 = mod(z * z, p3);
  let z4 = mod(z2 * z2, p3);
  let z6 = mod(z4 * z2, p3);
  return mod(y2 - x3 - a2 * x * z4 - b2 * z6, p3) === 0n;
}
function projectiveInSubgroup(g, p3, order, a2) {
  let orderTimesG = projectiveScale(g, order, p3, a2);
...
function createCurveProjective({ name, modulus: p3, order, cofactor, generator, b: b2, a: a2, endoBase, endoScalar }) {
  ...
  return {
    ...
    zero: projectiveZero,
    one: { ...generator, z: 1n },
    hasEndomorphism: endoBase !== void 0 && endoScalar !== void 0,
    get endoBase() {
      if (endoBase === void 0)
    ...
    isOnCurve(g) {
      return projectiveOnCurve(g, p3, b2, a2);
    },
    isInSubgroup(g) {
      return projectiveInSubgroup(g, p3, order, a2);
    },
    add(g, h) {
      return projectiveAdd(g, h, p3, a2);
    },
    double(g) {
      return double2(g, p3);
    },
    ...
    sub(g, h) {
      return projectiveSub(g, h, p3, a2);
    },
    scale(g, s) {
      return projectiveScale(g, s, p3, a2);
    },
    endomorphism({ x, y, z }) {
      if (endoBase === void 0)
...
function affineDouble({ x, y, infinity }, p3, a2) {
  if (infinity)
    return affineZero;
  let d = inverse(2n * y, p3);
  if (d === void 0)
    throw Error("impossible");
  let m = mod((3n * x * x + a2) * d, p3);
...
function affineScale(g, s, p3, a2) {
  let gProj = projectiveFromAffine(g);
  let sgProj = projectiveScale(gProj, s, p3, a2);
...
function createCurveAffine({ name, modulus: p3, order, cofactor, generator, a: a2, b: b2, endoScalar, endoBase }) {
  ...
  const Endo = Endomorphism(Field5, Scalar4, one, a2, endoScalar, endoBase);
  return {
    name,
    /**
    ...
    isInSubgroup(g) {
      return projectiveInSubgroup(projectiveFromAffine(g), p3, order, a2);
    },
    add(g, h) {
      return affineAdd(g, h, p3, a2);
    },
    double(g) {
      return affineDouble(g, p3, a2);
    },
    ...
    sub(g, h) {
      return affineAdd(g, affineNegate(h, p3), p3, a2);
    },
    scale(g, s) {
      return affineScale(g, s, p3, a2);
    }
  };
}
...
function srsPerField(f, wasm2, conversion) {
  ...
  let lagrangeCommitment = (srs2, domain_size, i) => wasm2[`caml_${f}_srs_lagrange_commitment`](srs2, domain_size, i);
...
function createDerivers() {
  ...
  function hasProvable(typeObj) {
    return "provable" in typeObj && (typeof typeObj.provable === "object" || typeof typeObj.provable === "function") && typeObj.provable !== null && isProvable2(typeObj.provable);
  }
  function provable3(typeObj) {
    ...
    function toFields(typeObj2, obj) {
      ...
      if (Array.isArray(typeObj2)) {
        ...
        if (typeObj2.length !== obj.length) {
          throw Error(`Expected array length ${typeObj2.length}, but got ${obj.length}`);
        }
        return typeObj2.map((t, i) => toFields(t, obj[i])).flat();
      }
      if (isProvable2(typeObj2))
...
`);
    ...
    function toCanonical2(typeObj2, value) {
      if (isPrimitive(typeObj2))
        return value;
      if (!complexTypes.has(typeof typeObj2))
        throw Error(`provable: unsupported type "${typeObj2}"`);
      if (hasProvable(typeObj2))
        return typeObj2.provable.toCanonical?.(value) ?? value;
      if (Array.isArray(typeObj2)) {
        return typeObj2.forEach((t, i) => toCanonical2(t, value[i]));
      }
      if (isProvable2(typeObj2))
        return typeObj2.toCanonical?.(value) ?? value;
      return Object.fromEntries(Object.keys(typeObj2).map((k) => {
        return [k, toCanonical2(typeObj2[k], value[k])];
      }));
    }
    ...
    let { empty: empty4, fromJSON, toJSON, toInput } = signable2(
      ...
      (obj) => isProvable2(obj) || hasProvable(obj)
    );
    const type = typeObj;
    return {
      ...
      fromFields: (fields2, aux) => fromFields(type, fields2, aux),
...
function arrayGet(array, index) {
  if (index.isConstant()) {
    return array[Number(index.toBigInt())] ?? createField2(0n);
  }
  let i = toVar(index);
  let a2 = existsOne(() => array[Number(i.toBigInt())]?.toBigInt() ?? 0n);
  let n = array.length;
  for (let j = 0; j < n; j++) {
    let zj = existsOne(() => {
...
function assertNotVectorEquals(x, c) {
  let xv = x.map(toVar);
  let n = c.length;
  assert2(n > 1 && x.length === n, "vector lengths must match");
  let z = exists(n, () => {
...
var ProvableType = {
  ...
  synthesize(type) {
    ...
    return provable3.fromFields(Array(provable3.sizeInFields()).fill(createField2(0)), provable3.toAuxiliary());
  }
};
...
function mapValue(provable3, there, back) {
  return {
    sizeInFields() {
      return provable3.sizeInFields();
    },
    toFields(value) {
      return provable3.toFields(value);
    },
    toAuxiliary(value) {
      return provable3.toAuxiliary(value);
    },
    fromFields(fields2, aux) {
      return provable3.fromFields(fields2, aux);
    },
    check(value) {
      provable3.check(value);
    },
    toValue(value) {
      return there(provable3.toValue(value));
    },
    fromValue(value) {
      return provable3.fromValue(back(value));
    },
    empty() {
      return provable3.empty();
    },
    toInput(value) {
      return provable3.toInput(value);
    }
  };
}
...
function equal(type, x, y) {
  let provable3 = ProvableType.get(type);
  x = provable3.toCanonical?.(x) ?? x;
  y = provable3.toCanonical?.(y) ?? y;
  let xs = provable3.toFields(x);
  let ys = provable3.toFields(y);
...
`);
    assert(this.option.isSome, "Empty `Unconstrained`");
...
function rangeCheck16(x) {
  if (x.isConstant()) {
    assert3(x.toBigInt() < 1n << 16n, `rangeCheck16: expected field to fit in 16 bits, got ${x}`);
    return;
  }
...
function rangeCheckLessThan64(bits, x) {
  assert3(bits < 64, `bits must be less than 64, got ${bits}`);
  if (x.isConstant()) {
    assert3(x.toBigInt() < 1n << BigInt(bits), `rangeCheckLessThan16: expected field to fit in ${bits} bits, got ${x}`);
    return;
  }
  rangeCheck64(x);
...
function toCanonical(x, f) {
  let xR = multiply(x, Field32.from(1n), f);
  assertLessThan(xR, f);
  return xR;
}
...
function shiftedScalarToField3(t) {
  if (t.high254.isConstant() && t.lowBit.isConstant()) {
    let s = t.lowBit.toField().toBigInt() + 2n * t.high254.toBigInt();
    return Field32.from(Fq.mod(s + (1n << 255n)));
  }
  let [t0Hi0, t0Hi1, t1, t2] = exists(4, () => {
    let tHi2 = t.high254.toBigInt();
    return [
      bitSlice(tHi2, 0, 64),
      bitSlice(tHi2, 64, 23),
      bitSlice(tHi2, 87, 88),
      bitSlice(tHi2, 175, 79)
    ];
  });
  rangeCheck64(t0Hi0);
  rangeCheckLessThan64(23, t0Hi1);
  let t0Hi = t0Hi0.add(t0Hi1.mul(1n << 64n)).seal();
  let t0 = t.lowBit.toField().add(t0Hi.mul(2n)).seal();
  multiRangeCheck([t1, t2, t2.mul(1n << 9n)]);
  let tHi = t0Hi.add(t1.mul(1n << 87n)).add(t2.mul(1n << 175n)).seal();
  tHi.assertEquals(t.high254);
  let tBig = [t0, t1, t2];
  let shift = Fq.mod(1n << 255n);
  return ForeignField.sum([tBig, Field32.from(shift), Field32.from(0n)], [1n, 1n], Fq.modulus);
}
function add(g, h) {
...
var Scalar3 = class _Scalar {
   ...
   */
  static fromFields(fields2) {
    assert2(fields2.length === 2, `Scalar.fromFields(): expected 2 fields, got ${fields2.length}`);
  ...
  static toCanonical(s) {
    ...
    return _Scalar.fromShiftedScalar(sCanonical);
  }
  static toValue(x) {
    return x.toBigInt();
  }
...
function assertConstant(x, name) {
  assert2(x.isConstant(), `${name}() is not available in provable code.
...
var Group2 = class _Group {
  ...
  addNonZero(g2, allowZeroOutput = false) {
    if (isConstant5(this) && isConstant5(g2)) {
      ...
      assert2(!infinity || allowZeroOutput, "Group.addNonzero(): Result is zero");
...
// dist/node/lib/proof-system/feature-flags.js
var FeatureFlags = {
  /**
...
// dist/node/lib/proof-system/proof.js
var ProofBase = class {
  toJSON() {
    let fields2 = this.publicFields();
    return {
      publicInput: fields2.input.map(String),
      publicOutput: fields2.output.map(String),
  ...
  static get provable() {
    if (this.publicInputType === void 0 || this.publicOutputType === void 0) {
...
class MyProof extends Proof<PublicInput, PublicOutput> { ... }`);
  ...
  static publicFields(value) {
    let fields2 = this.provable.toFields(value);
    let inputSize = this.publicInputType.sizeInFields();
    return {
      input: fields2.slice(0, inputSize),
      output: fields2.slice(inputSize)
    };
  }
  publicFields() {
...
var Proof = class extends ProofBase {
  ...
  static async fromJSON({ maxProofsVerified, proof: proofString, publicInput: publicInputJson, publicOutput: publicOutputJson }) {
    ...
    let fields2 = publicInputJson.map(Field4).concat(publicOutputJson.map(Field4));
    return this.provable.fromFields(fields2, [
      [],
      [],
...
function provableProof(Class, input, output, defaultMaxProofsVerified) {
  return {
    ...
    fromFields(fields2, aux) {
      let inputFields = fields2.slice(0, input.sizeInFields());
      let outputFields = fields2.slice(input.sizeInFields());
      assert2(outputFields.length === output.sizeInFields());
...
function assertBindingsInitialized() {
  assert2(isInitialized, "Bindings are not initialized. Try calling `await initializeBindings()` first.");
}
...
function or(a2, b2, length) {
  return not(and(not(a2, length), not(b2, length), length), length);
}
...
var UInt642 = class _UInt64 extends CircuitValue {
   ...
   */
  addMod64(y) {
    return new _UInt64(addMod64(this.value, y.value).value);
  }
  /**
   ...
   */
  or(x) {
    return new _UInt64(or(this.value, x.value, _UInt64.NUM_BITS).value);
  }
  /**
  ...
  assertLessThanOrEqual(y, message) {
    if (this.value.isConstant() && y.value.isConstant()) {
      ...
      return assert2(x0 <= y0, message ?? `UInt64.assertLessThanOrEqual: expected ${x0} <= ${y0}`);
    }
  ...
  assertLessThan(y, message) {
    if (this.value.isConstant() && y.value.isConstant()) {
      let [x0, y0] = [this.value.toBigInt(), y.value.toBigInt()];
      return assert2(x0 < y0, message ?? `UInt64.assertLessThan: expected ${x0} < ${y0}`);
    }
...
var UInt322 = class _UInt32 extends CircuitValue {
   ...
   */
  or(x) {
    return new _UInt32(or(this.value, x.value, _UInt32.NUM_BITS).value);
  }
  /**
  ...
  assertLessThanOrEqual(y, message) {
    if (this.value.isConstant() && y.value.isConstant()) {
      let [x0, y0] = [this.value.toBigInt(), y.value.toBigInt()];
      return assert2(x0 <= y0, message ?? `UInt32.assertLessThanOrEqual: expected ${x0} <= ${y0}`);
    }
  ...
  lessThan(y) {
    ...
    return lessThanGeneric(this.value, y.value, 1n << 32n, (v) => rangeCheckN(_UInt32.NUM_BITS, v));
  }
  /**
  ...
  assertLessThan(y, message) {
    if (this.value.isConstant() && y.value.isConstant()) {
      let [x0, y0] = [this.value.toBigInt(), y.value.toBigInt()];
      return assert2(x0 < y0, message ?? `UInt32.assertLessThan: expected ${x0} < ${y0}`);
    }
   ...
   */
  static fromBytes(bytes) {
    assert2(bytes.length === 4, "4 bytes needed to create a uint32");
    return _UInt32.Unsafe.fromField(bytesToWord(bytes));
  }
  /**
   * Combine 4 UInt8s into a UInt32, in big-endian order.
   */
  static fromBytesBE(bytes) {
    return _UInt32.fromBytes([...bytes].reverse());
  }
};
UInt322.NUM_BITS = 32;
UInt322.Unsafe = {
...
var Int64 = class _Int64 extends CircuitValue {
  ...
  static fromFieldUnchecked(x) {
    ...
    return _Int64.create(UInt642.Unsafe.fromField(magnitude), sign3);
  }
  ...
  static fromUnsigned(x) {
    return _Int64.create(x instanceof UInt322 ? x.toUInt64() : x);
  }
  // this checks the range if the argument is a constant
  /**
   ...
   */
  toBigint() {
    let abs2 = this.magnitude.toBigInt();
   ...
   */
  static get zero() {
    return _Int64.create(UInt642.zero);
  }
  /**
   * Static method to create a {@link Int64} with value `1`.
   */
  static get one() {
    return _Int64.create(UInt642.one);
  }
  /**
   * Static method to create a {@link Int64} with value `-1`.
   */
  static get minusOne() {
    return _Int64.create(UInt642.one).neg();
  }
  /**
...
var UInt8 = class _UInt8 extends Struct({
  ...
  assertLessThan(y, message) {
    ...
    if (this.value.isConstant() && y_.value.isConstant()) {
      ...
      return assert2(x0 < y0, message ?? `UInt8.assertLessThan: expected ${x0} < ${y0}`);
    }
    try {
  ...
  assertLessThanOrEqual(y, message) {
    ...
    if (this.value.isConstant() && y_.value.isConstant()) {
      let [x0, y0] = [this.value.toBigInt(), y_.value.toBigInt()];
      return assert2(x0 <= y0, message ?? `UInt8.assertLessThanOrEqual: expected ${x0} <= ${y0}`);
    }
    try {
...
function padding(data) {
  ...
  for (let i = 0; i < paddedMessage.length; i += 4) {
    chunks.push(UInt322.fromBytesBE(paddedMessage.slice(i, i + 4)));
  }
...
var SHA256 = {
  hash(data) {
    ...
    return Bytes2.from(H.map((x) => x.toBytesBE()).flat());
  },
...
function sha256Compression([...H], W) {
  ...
  for (let t = 0; t <= 63; t++) {
    ...
    g = f;
    f = e;
    e = UInt322.Unsafe.fromField(divMod32(d.value.add(unreducedT1), 48).remainder);
    d = c;
    c = b2;
    b2 = a2;
    a2 = UInt322.Unsafe.fromField(divMod32(unreducedT2.add(unreducedT1), 48).remainder);
  }
  H[0] = H[0].addMod32(a2);
  H[1] = H[1].addMod32(b2);
  H[2] = H[2].addMod32(c);
...
function createMessageSchedule(M) {
  ...
  for (let t = 16; t <= 63; t++) {
    ...
    W[t] = UInt322.Unsafe.fromField(divMod32(unreduced, 48).remainder);
  }
  return W;
}

// dist/node/lib/provable/gadgets/blake2b.js
var BLAKE2BConstants = {
  IV: [
    UInt642.from(0x6a09e667f3bcc908n),
    UInt642.from(0xbb67ae8584caa73bn),
    UInt642.from(0x3c6ef372fe94f82bn),
    UInt642.from(0xa54ff53a5f1d36f1n),
    UInt642.from(0x510e527fade682d1n),
    UInt642.from(0x9b05688c2b3e6c1fn),
    UInt642.from(0x1f83d9abfb41bd6bn),
    UInt642.from(0x5be0cd19137e2179n)
  ],
  SIGMA: [
...
var BLAKE2B = {
  hash(data, digestLength = 64) {
    ...
    const state3 = initialize(digestLength);
    const updatedState = update(state3, Bytes2.from(data).bytes);
    const out = final(updatedState);
    return Bytes2.from(out);
  },
  get IV() {
    return BLAKE2BConstants.IV;
  }
};
function G(v, a2, b2, c, d, x, y) {
  v[a2] = UInt642.Unsafe.fromField(Gadgets.divMod64(v[a2].value.add(v[b2].value.add(x.value)), 128).remainder);
  v[d] = v[d].xor(v[a2]).rotate(32, "right");
  v[c] = UInt642.Unsafe.fromField(Gadgets.divMod64(v[c].value.add(v[d].value), 128).remainder);
  v[b2] = v[b2].xor(v[c]).rotate(24, "right");
  v[a2] = UInt642.Unsafe.fromField(Gadgets.divMod64(v[a2].value.add(v[b2].value.add(y.value)), 128).remainder);
  v[d] = v[d].xor(v[a2]).rotate(16, "right");
  v[c] = UInt642.Unsafe.fromField(Gadgets.divMod64(v[c].value.add(v[d].value), 128).remainder);
  v[b2] = v[b2].xor(v[c]).rotate(63, "right");
}
function compress(state3, last) {
  const { h, t, buf } = state3;
  const v = h.concat(BLAKE2B.IV);
  v[12] = v[12].xor(UInt642.from(t[0]));
  v[13] = v[13].xor(UInt642.from(t[1]));
  if (last) {
    v[14] = v[14].not();
  }
  const m = [];
  for (let i = 0; i < 16; i++) {
    m.push(UInt642.Unsafe.fromField(buf[i * 8].value.add(buf[i * 8 + 1].value.mul(1n << 8n)).add(buf[i * 8 + 2].value.mul(1n << 16n)).add(buf[i * 8 + 3].value.mul(1n << 24n)).add(buf[i * 8 + 4].value.mul(1n << 32n)).add(buf[i * 8 + 5].value.mul(1n << 40n)).add(buf[i * 8 + 6].value.mul(1n << 48n)).add(buf[i * 8 + 7].value.mul(1n << 56n)).seal()));
  }
  for (let i = 0; i < 12; i++) {
...
function initialize(outlen) {
  const h = BLAKE2B.IV.slice();
  h[0] = UInt642.from(16842752).xor(UInt642.from(outlen)).xor(h[0]);
  return {
    h,
    t: [0n, 0n],
    buf: [],
...
function update(state3, input) {
  for (let i = 0; i < input.length; i++) {
    if (state3.buflen === 128) {
      state3.t[0] = UInt642.from(state3.t[0]).addMod64(UInt642.from(state3.buflen)).toBigInt();
      if (state3.t[0] < state3.buflen) {
        state3.t[1] = UInt642.from(state3.t[1]).addMod64(UInt642.one).toBigInt();
      }
      state3 = compress(state3, false);
      state3.buflen = 0;
    }
    state3.buf[state3.buflen++] = input[i];
  }
  return state3;
}
function final(state3) {
  state3.t[0] = UInt642.from(state3.t[0]).addMod64(UInt642.from(state3.buflen)).toBigInt();
  if (state3.t[0] < state3.buflen) {
    state3.t[1] = UInt642.from(state3.t[1]).addMod64(UInt642.one).toBigInt();
  }
  while (state3.buflen < 128) {
    state3.buf[state3.buflen++] = UInt8.from(0);
  }
  compress(state3, true);
  const out = state3.h.slice(0, state3.outlen / 8).flatMap((x) => wordToBytes(x.value));
  return out;
}
...
var ForeignField2 = class _ForeignField {
  ...
  constructor(x) {
    ...
    if (x instanceof _ForeignField) {
      if (x.modulus !== p3) {
        throw new Error(`ForeignField constructor: modulus mismatch. Expected ${p3}, got ${x.modulus}. Please provide a value with the correct modulus. You can use 'Gadgets.ForeignField.assertLessThan()' to check it.`);
      }
      this.value = x.value;
      return;
    }
  ...
  return {
    ...
    toCanonical(x) {
      if (x.type === "FullyReduced")
        return x;
      return new Class(ForeignField.toCanonical(x.value, x.modulus));
    },
    toValue(x) {
      return x.toBigInt();
    },
...
function add2(p1, p22, Curve) {
  ...
  if (Point.isConstant(p1) && Point.isConstant(p22)) {
    let p3 = affineAdd(Point.toBigint(p1), Point.toBigint(p22), f, Curve.a);
...
function double(p1, Curve) {
  ...
  let witnesses = exists(9, () => {
    ...
    let m3 = mod((3n * mod(x1_ ** 2n, f) + Curve.a) * denom, f);
...
function verifyEcdsaGeneric(Curve, signature, msgHash, publicKey, multiScalarMul2, config = { G: { windowSize: 4 }, P: { windowSize: 4 } }) {
  ...
  let G2 = Point.from(Curve.one);
  let R = multiScalarMul2([u1, u2], [G2, publicKey], Curve, config && [config.G, config.P], "assert-nonzero", config?.ia);
...
function verifyEcdsa(Curve, signature, msgHash, publicKey, config = { G: { windowSize: 4 }, P: { windowSize: 3 } }) {
  return verifyEcdsaGeneric(Curve, signature, msgHash, publicKey, (scalars, points, Curve2, configs, mode, ia) => multiScalarMul(scalars, points, Curve2, configs, mode, ia), config);
}
...
var ForeignCurve = class {
  ...
  static fromHex(hex) {
    ...
    const xBytes = tail.subarray(0, sizeInBytes);
    const yBytes = tail.subarray(sizeInBytes, 2 * sizeInBytes);
  ...
  static fromEthers(hex) {
    ...
    const xBytes = tail.slice(0, 32);
    const x = BigInt("0x" + Bytes.from(xBytes).toHex());
    let p3 = void 0;
    if (len === 33 && [2, 3].includes(head)) {
      assert3(0n < x && x < this.Bigint.Field.modulus);
      ...
      if (headOdd !== isYOdd)
        y = this.Bigint.Field.mod(-y);
      p3 = { x, y };
    }
    if (len === 65 && head === 4) {
      const yBytes = tail.slice(32, 64);
      p3 = { x, y: BigInt("0x" + Bytes.from(yBytes).toHex()) };
    }
    const P = this.from(p3);
    P.assertOnCurve();
    return P;
  }
  /**
...
var State = {
   ...
   */
  xor(a2, b2) {
    assert(a2.length === KECCAK_DIM && a2[0].length === KECCAK_DIM, `invalid \`a\` dimensions (should be ${KECCAK_DIM})`);
    assert(b2.length === KECCAK_DIM && b2[0].length === KECCAK_DIM, `invalid \`b\` dimensions (should be ${KECCAK_DIM})`);
...
var EcdsaSignature2 = class {
  ...
  verifyEthers(message, publicKey) {
    ...
    const msgHashBytes = Keccak.ethereum([
      ...Bytes2.fromString(MessagePrefix).bytes,
      // prefix for Ethereum signed messages
      ...Bytes2.fromString(String(message.length)).bytes,
...
function createPoseidon(Fp2, { fullRounds, partialRounds, hasInitialRoundConstant, stateSize, rate, power: power_, roundConstants: roundConstants_, mds: mds_ }) {
  ...
  function hash3(input) {
    let state3 = update2(initialState(), input);
    return state3[0];
  }
  function update2([...state3], input) {
    if (input.length === 0) {
      permutation2(state3);
      return state3;
    }
  ...
  return { initialState, update: update2, hash: hash3 };
}
...
var Sponge = class {
  ...
  constructor() {
    ...
    assert(Snarky !== void 0, "Poseidon.Sponge(): bindings are not initialized, try calling `await initializeBindings()` first.");
    (0, import_tslib3.__classPrivateFieldSet)(this, _Sponge_sponge, Snarky.poseidon.sponge.create(isChecked), "f");
  }
  absorb(x) {
    Snarky.poseidon.sponge.absorb((0, import_tslib3.__classPrivateFieldGet)(this, _Sponge_sponge, "f"), x.value);
  }
  squeeze() {
    return Field4(Snarky.poseidon.sponge.squeeze((0, import_tslib3.__classPrivateFieldGet)(this, _Sponge_sponge, "f")));
  }
};
_Sponge_sponge = /* @__PURE__ */ new WeakMap();
var Poseidon2 = {
  hash(input) {
    if (isConstant7(input)) {
      return Field4(Poseidon.hash(toBigints(input)));
    }
    return Poseidon2.update(Poseidon2.initialState(), input)[0];
  },
  update(state3, input) {
    if (isConstant7(state3) && isConstant7(input)) {
...
var Packed = class _Packed {
  ...
  static create(type) {
    ...
    return _a2 = class Packed_ extends _Packed {
      static pack(x) {
        let input2 = provable3.toInput(x);
        let packed = packToFields(input2);
        let unconstrained = Unconstrained.witness(() => Provable.toConstant(provable3, x));
        return new _a2(packed, unconstrained);
      }
      static empty() {
    ...
    }, _a2._innerProvable = provable3, _a2._provable = mapValue(provableFromClass(_a2, {
      packed: packedFields,
      value: Unconstrained
    }), ({ value }) => provable3.toValue(value.get()), (x) => {
      if (x instanceof _Packed)
        return x;
      let { packed, value } = _a2.pack(provable3.fromValue(x));
      return {
...
var MayUseToken = {
  ...provable({ parentsOwnToken: Bool4, inheritFromParent: Bool4 }),
  check: ({ parentsOwnToken, inheritFromParent }) => {
    Bool4.check(parentsOwnToken);
    Bool4.check(inheritFromParent);
    parentsOwnToken.and(inheritFromParent).assertFalse("MayUseToken: parentsOwnToken and inheritFromParent cannot both be true");
  }
};
...
function Option(type) {
  ...
  return class Option_ extends Super {
    ...
    static from(value) {
      return value === void 0 ? new Option_({
        isSome: Bool4(false),
        value: ProvableType.synthesize(strictType)
      }) : new Option_({
        isSome: Bool4(true),
...
function getProvableType(layout) {
  ...
  assert(type !== void 0, `Unknown precondition base type ${typeName}`);
  return type;
}
...
function defaultLower(fieldType) {
  assert(fieldType === UInt322 || fieldType === UInt642);
...
function ZkProgram(config) {
  ...
  async function compile({ cache: cache2 = Cache.FileSystemDefault, forceRecompile = false, proofsEnabled = void 0 } = {}) {
  ...
  function toProver(key, i) {
    async function prove_(publicInput, ...args) {
      ...
      if (!doProving) {
        let previousProofs2 = MlArray.to(getPreviousProofsForProver(args));
        let { publicOutput: publicOutput2, auxiliaryOutput: auxiliaryOutput2 } = await methods[key].method(publicInput, previousProofs2) ?? {};
        let proof2 = await ProgramProof.dummy(publicInput, publicOutput2, maxProofsVerified);
...
async function compileProgram({ publicInputType, publicOutputType, methodIntfs, methods, gates, proofSystemTag, cache: cache2, forceRecompile, overrideWrapDomain, state: state3 }) {
  await initializeBindings();
...
If you are using a SmartContract, make sure you are using the @method decorator.`);
  let rules = methodIntfs.map((methodEntry, i) => picklesRuleFromFunction(publicInputType, publicOutputType, methods[i], proofSystemTag, methodEntry, gates[i], state3));
...
function picklesRuleFromFunction(publicInputType, publicOutputType, func, proofSystemTag, { methodName, args, auxiliaryType }, gates, state3) {
  async function main(publicInput) {
    ...
    for (let i = 0; i < args.length; i++) {
      ...
      try {
        ...
        for (let proof of extractProofs(value)) {
          let Proof3 = proof.constructor;
          proofs2.push({ Proof: Proof3, proof });
          let fields2 = proof.publicFields();
    ...
    if (result?.publicOutput) {
      ...
      state3?.setNonPureOutput(nonPureOutput);
    }
    proofs2.forEach(({ Proof: Proof3, proof }) => {
      if (!(proof instanceof DynamicProof))
        return;
      const tag = Proof3.tag();
    ...
    if (state3 !== void 0 && auxiliaryType !== void 0 && auxiliaryType.sizeInFields() !== 0) {
      Provable.asProver(() => {
        let { auxiliaryOutput } = result;
        assert(auxiliaryOutput !== void 0, `${proofSystemTag.name}.${methodName}(): Auxiliary output is undefined even though the method declares it.`);
        state3.setAuxiliaryOutput(Provable.toConstant(auxiliaryType, auxiliaryOutput), methodName);
      });
    }
    return {
      publicOutput: MlFieldArray.to(publicOutput),
      previousStatements: MlArray.to(previousStatements),
      shouldVerify: MlArray.to(proofs2.map((proof) => proof.proof.shouldVerify.toField().value))
    };
  }
  let proofs = args.flatMap(extractProofTypes);
  if (proofs.length > 2) {
...
Suggestion: You can merge more than two proofs by merging two at a time in a binary tree.`);
  ...
  let proofsToVerify = proofs.map((Proof3) => {
    let tag = Proof3.tag();
...
function verifyAccountUpdateSignature(update2, transactionCommitments2, networkId) {
  if (update2.authorization.signature === void 0)
    return false;
  let { publicKey, useFullCommitment } = update2.body;
  let { commitment, fullCommitment } = transactionCommitments2;
  let usedCommitment = useFullCommitment ? fullCommitment : commitment;
  let signature = Signature.fromBase58(update2.authorization.signature);
...
function accountUpdateHash(update2, networkId) {
  assertAuthorizationKindValid(update2);
  let input = AccountUpdate2.toInput(update2);
...
function contract(expectedConstructor) {
  ...
  assert2(ctx !== null, "This method must be called within a contract method");
  if (expectedConstructor !== void 0) {
    assert2(ctx.this.constructor === expectedConstructor, `This method must be called on a ${expectedConstructor.name} contract`);
  }
  return ctx.this;
}
...
var AccountUpdate3 = class _AccountUpdate {
  ...
  send({ to, amount }) {
    ...
    if (to instanceof _AccountUpdate) {
      ...
      receiver = _AccountUpdate.default(to, this.body.tokenId);
  ...
  static getSigningInfoUnchecked(update2) {
    let publicKey = update2.body.publicKey;
    let tokenId = update2 instanceof _AccountUpdate ? update2.body.tokenId : TokenId4.default;
    let nonce = Number(getAccountPreconditions(update2.body).nonce.toString());
    ...
    layout?.forEachPredecessor(update2, (otherUpdate) => {
  ...
  toPublicInput({ accountUpdates }) {
    ...
    assert2(i !== -1, "Account update not found in transaction");
    for (i++; i < accountUpdates.length; i++) {
      let update2 = accountUpdates[i];
      if (update2.body.callDepth <= callDepth)
        break;
      descendants.push(update2);
    }
  ...
  toPrettyLayout() {
    ...
    assert2(node !== void 0, "AccountUpdate not found in layout");
    node.children.print();
  }
  extractTree() {
...
function hashAccountUpdate(update2) {
  return genericHash(AccountUpdate3, zkAppBodyPrefix(activeInstance.getNetworkId()), update2);
}
...
var AccountUpdateForest = class extends (_b = MerkleList.create(AccountUpdateTreeBase, merkleListHash2)) {
  push(update2) {
    return super.push(update2 instanceof AccountUpdate3 ? AccountUpdateTree.from(update2) : update2);
  }
  pushIf(condition, update2) {
    return super.pushIf(condition, update2 instanceof AccountUpdate3 ? AccountUpdateTree.from(update2) : update2);
  }
  ...
  static toFlatArray(forest, mutate = true, depth = 0) {
    ...
    for (let { element: tree } of forest.data.get()) {
      let update2 = tree.accountUpdate.value.get();
      if (mutate)
        update2.body.callDepth = depth;
      flat.push(update2);
  ...
  static assertConstant(forest) {
    Provable.asProver(() => {
      forest.data.get().forEach(({ element: tree }) => {
        assert2(Provable.isConstant(AccountUpdate3, tree.accountUpdate.value.get()), "account update not constant");
...
var AccountUpdateTree = class _AccountUpdateTree extends StructNoJson({
   ...
   */
  static from(update2, hash3) {
    if (update2 instanceof _AccountUpdateTree)
      return update2;
    return new _AccountUpdateTree({
      accountUpdate: HashedAccountUpdate.hash(update2, hash3),
      id: update2.id,
  ...
  approve(update2, hash3) {
    ...
    if (update2 instanceof AccountUpdate3) {
...
var UnfinishedForest = class _UnfinishedForest {
  ...
  constructor(mutable, final2) {
    assert2(final2 === void 0 !== (mutable === void 0), "final or mutable");
    this.final = final2;
  ...
  setFinal(final2) {
    return Object.assign(this, { final: final2, mutable: void 0 });
  }
  finalize() {
    if (this.isFinal())
      return this.final;
    assert2(this.isMutable(), "final or mutable");
  ...
  witnessHash() {
    let final2 = Provable.witness(AccountUpdateForest, () => this.finalize());
    return this.setFinal(final2);
  }
  push(node) {
    if (node.siblings === this)
      return;
    assert2(node.siblings === void 0, "Cannot push node that already has a parent.");
    node.siblings = this;
    assert2(this.isMutable(), "Cannot push to an immutable forest");
    this.mutable.push(node);
  }
  remove(node) {
    assert2(this.isMutable(), "Cannot remove from an immutable forest");
  ...
  setToForest(forest) {
    if (this.isMutable()) {
      assert2(this.mutable.length === 0, "Replacing a mutable forest that has existing children might be a mistake.");
    }
  ...
  toFlatArray(mutate = true, depth = 0) {
    ...
    for (let node of this.mutable) {
      if (node.isDummy.toBoolean())
        continue;
      let update2 = node.mutable ?? node.final.value.get();
      if (mutate)
        update2.body.callDepth = depth;
      let children = node.children.toFlatArray(mutate, depth + 1);
      flatUpdates.push(update2, ...children);
    }
    return flatUpdates;
  }
  toConstantInPlace() {
  ...
  print() {
    ...
    let toPretty = (a2) => {
      ...
      assert2(a2.isMutable(), "final or mutable");
      indent += 2;
...
var UnfinishedTree = {
  create(update2) {
    ...
    return {
      final: update2.accountUpdate,
      id: update2.id,
      isDummy: Bool4(false),
      children: UnfinishedForest.fromForest(update2.children)
    };
  },
  setTo(node, update2) {
    if (update2 instanceof AccountUpdate3) {
...
var AccountUpdateLayout = class {
  ...
  get(update2) {
    return this.map.get(update2.id);
  }
  getOrCreate(update2) {
    if (UnfinishedTree.isUnfinished(update2)) {
      if (!this.map.has(update2.id)) {
        this.map.set(update2.id, update2);
      }
      return update2;
    }
    let node = this.map.get(update2.id);
    if (node !== void 0) {
      UnfinishedTree.setTo(node, update2);
      return node;
    }
    node = UnfinishedTree.create(update2);
    this.map.set(update2.id, node);
    return node;
  }
  ...
  disattach(update2) {
    let node = this.get(update2);
    node?.siblings?.remove(node);
    return node;
  }
  finalizeAndRemove(update2) {
    let node = this.get(update2);
    if (node === void 0)
      return;
    this.disattach(update2);
    return node.children.finalize();
  }
  finalizeChildren() {
    let final2 = this.root.children.finalize();
    this.final = final2;
    AccountUpdateForest.assertConstant(final2);
    return final2;
  }
  toFlatList({ mutate }) {
    return this.root.children.toFlatArray(mutate);
  }
  forEachPredecessor(update2, callback) {
    let updates = this.toFlatList({ mutate: false });
    for (let otherUpdate of updates) {
      if (otherUpdate.id === update2.id)
        return;
...
var defaultErrorReplacementRules = [
  {
...
function humanizeErrors(errors, replacements = defaultErrorReplacementRules) {
...
function verifyTransactionLimits({ accountUpdates }) {
  ...
  let authKinds = accountUpdates.map((update2) => {
    eventElements.events += countEventElements(update2.body.events);
    eventElements.actions += countEventElements(update2.body.actions);
    let { isSigned, isProved, verificationKeyHash } = update2.body.authorizationKind;
    return {
...
async function verifyAccountUpdate(account, accountUpdate, publicInput, transactionCommitments2, proofsEnabled, networkId) {
  ...
  const update2 = accountUpdateJson.body.update;
  let errorTrace = "";
  ...
  if (accountUpdate.authorization.proof && proofsEnabled) {
    try {
      ...
      isValidProof = await verify2(proof, verificationKey);
      if (!isValidProof) {
        throw Error(`Invalid proof for account update
${JSON.stringify(update2)}`);
      }
    } catch (error) {
      errorTrace += "\n\n" + error.stack;
      isValidProof = false;
...
function applyAccountUpdate(account, update2) {
  account.publicKey.assertEquals(update2.publicKey);
  account.tokenId.assertEquals(update2.tokenId, "token id mismatch");
  let json = Account4.toJSON(account);
  account = Account4.fromJSON(json);
  if (update2.update.permissions.isSome.toBoolean()) {
    account.permissions = update2.update.permissions.value;
  }
  return account;
}
...
var SimpleLedger = class _SimpleLedger {
  ...
  apply(update2) {
    let id2 = accountId(update2.body);
    let account = this.accounts.get(id2);
    account ??= newAccount(update2.body);
    let updated = applyAccountUpdate(account, update2);
  ...
  return {
    ...
    sendTransaction(txn) {
      return toPendingTransactionPromise(async () => {
        ...
        for (const update2 of txn.transaction.accountUpdates) {
          let authIsProof = !!update2.authorization.proof;
          let kindIsProof = update2.body.authorizationKind.isProved.toBoolean();
          ...
          let account = simpleLedger.load(update2.body);
          if (account === void 0) {
            let accountJson = ledger.getAccount(Ml.fromPublicKey(update2.body.publicKey), Ml.constFromField(update2.body.tokenId));
            if (accountJson !== void 0) {
          ...
          if (account !== void 0) {
            let publicInput = update2.toPublicInput(txn.transaction);
            await verifyAccountUpdate(account, update2, publicInput, commitments, this.proofsEnabled, this.getNetworkId());
            simpleLedger.apply(update2);
          }
        }
        let status = "pending";
        const errors = [];
...
function Network2(options) {
  ...
  if (options && typeof options === "string") {
    ...
    if (options.bypassTransactionLimits !== void 0 && typeof options.bypassTransactionLimits === "boolean") {
...
${err.message}`;
    ...
    async fetch() {
      ...
      if (networkConfig.minaEndpoint === "") {
        account = getAccount(address, tokenId);
      } else {
        ({ account } = await fetchAccount({
          publicKey: address,
          tokenId: TokenId4.toBase58(tokenId)
        }));
      }
      if (account === void 0)
        return void 0;
...
function method(target, methodName, descriptor, returnType) {
  ...
  let internalMethodEntry = sortMethodArguments(ZkappClass.name, methodName, paramTypes, void 0, SelfProof2);
  let methodEntry = sortMethodArguments(ZkappClass.name, methodName, [PublicKey2, Field4, ...paramTypes], void 0, SelfProof2);
  ...
  ZkappClass._maxProofsVerified = Math.max(ZkappClass._maxProofsVerified, methodEntry.numberOfProofs);
  ...
  return async function wrappedMethod(...actualArgs) {
        ...
        if (inCompile() || inProver() || inAnalyze()) {
          ...
          try {
            result = await assertPromise(method2.apply(this, actualArgs.map((a2, i) => {
              return Provable.witness(methodIntf.args[i], () => a2);
            })), noPromiseError);
          } finally {
    ...
    try {
      ...
      let runCalledContract = async () => {
        let constantArgs = methodIntf.args.map((type, i) => Provable.toConstant(type, actualArgs[i]));
...
var SmartContract = class extends SmartContractBase {
    ...
    this.sender = {
      ...
      getUnconstrained() {
        ...
        if ((0, import_tslib5.__classPrivateFieldGet)(this.self, _SmartContract__senderState, "f")?.transactionId === transactionId) {
          sender2 = (0, import_tslib5.__classPrivateFieldGet)(this.self, _SmartContract__senderState, "f").sender;
        } else {
          sender2 = Provable.witness(PublicKey2, () => sender());
          (0, import_tslib5.__classPrivateFieldSet)(this.self, _SmartContract__senderState, { transactionId, sender: sender2 }, "f");
        }
        sender2.x.assertNotEquals(0);
        return sender2;
      },
      /**
  ...
  async deploy({ verificationKey } = {}) {
    ...
    if (verificationKey === void 0) {
      if (!getProofsEnabled()) {
        verificationKey = await VerificationKey2.dummy();
      } else {
...
class ${this.constructor.name} extends SmartContract {
  ...
  async fetchEvents(start = UInt322.from(0), end) {
    ...
    if (sortedEventTypes.length === 0) {
      throw Error(`fetchEvents: You are trying to fetch events without having declared the types of your events.
Make sure to add a property \`events\` on ${this.constructor.name}, for example: 
class ${this.constructor.name} extends SmartContract {
  events = { 'my-event': Field }
}
...
then try Mina.fetchEvents('${this.address.toBase58()}') instead.`);
    ...
    if (start.greaterThan(UInt322.from(0)).toBoolean()) {
      queryFilterOptions.from = start;
    }
    if (end) {
      queryFilterOptions.to = end;
    }
    let events = (await fetchEvents(this.address, this.self.body.tokenId, queryFilterOptions)).map((event) => {
...
function selfAccountUpdate(zkapp, methodName) {
  ...
  let update2 = new AccountUpdate3(body, {}, true);
  update2.label = methodName ? `${zkapp.constructor.name}.${methodName}()` : `${zkapp.constructor.name}, no method`;
  return update2;
}
...
function diffRecursive(prover, inputData) {
  ...
  for (let i = index; i < transaction2.accountUpdates.length; i++) {
    let update2 = transaction2.accountUpdates[i];
    if (update2.body.callDepth <= callDepth)
      break;
    if (update2.body.callDepth === callDepth + 1)
      inputChildren.push(update2);
  }
...
var TokenAccountUpdateIterator = class _TokenAccountUpdateIterator {
  ...
  next() {
    ...
    let update2 = accountUpdate.unhash();
    let usesThisToken = update2.tokenId.equals(this.selfToken);
    let canAccessThisToken = Provable.equal(MayUseToken4.type, update2.body.mayUseToken, this.currentLayer.mayUseToken);
    let isSelf = TokenId4.derive(update2.publicKey, update2.tokenId).equals(this.selfToken);
    ...
    return { accountUpdate: update2, usesThisToken };
  }
...
function getApprovedUpdate(self, tokenId, child, label) {
  ...
  if (child instanceof PublicKey2) {
    child = AccountUpdate3.default(child, tokenId);
    self.approve(child);
  }
  if (!child.label)
...
TokenContract.MAX_ACCOUNT_UPDATES = 9;
...
function decrypt({ publicKey, cipherText }, privateKey) {
  ...
  const sponge2 = new Poseidon2.Sponge();
  sponge2.absorb(sharedSecret.x);
  const authenticationTag = cipherText.pop();
  const message = [];
  for (let i = 0; i < cipherText.length; i++) {
    if (i === cipherText.length - 1)
      sponge2.absorb(Field4(1));
    else
      sponge2.absorb(Field4(0));
    const keyStream = sponge2.squeeze();
...
function encrypt(message, otherPublicKey) {
  const privateKey = Provable.witness(Scalar3, () => Scalar3.random());
  const publicKey = Group3.generator.scale(privateKey);
  const sharedSecret = otherPublicKey.toGroup().scale(privateKey);
  const sponge2 = new Poseidon2.Sponge();
  sponge2.absorb(sharedSecret.x);
  const cipherText = [];
  for (let [n, chunk2] of message.entries()) {
    if (n === message.length - 1)
      sponge2.absorb(Field4(1));
    else
      sponge2.absorb(Field4(0));
    const keyStream = sponge2.squeeze();
    const encryptedChunk = chunk2.add(keyStream);
    cipherText.push(encryptedChunk);
    sponge2.absorb(encryptedChunk);
  }
  const authenticationTag = sponge2.squeeze();
...
function encryptBytes(message, otherPublicKey) {
  ...
  const padding2 = Array.from({ length: n - messageLength }, () => UInt8.from(0));
  const chunks = chunk(bytes.concat(padding2), 31);
  return {
    ...encrypt(chunks.map((chunk2) => bytesToWord(chunk2)), otherPublicKey),
    messageLength
  };
}
  ...
  return {
    ...
    async prove(tree, actions) {
      ...
      for (let i = 1; ; i++) {
        ...
        ({ proof, auxiliaryOutput: tree } = await offchainStateRollup.nextBatch(inputState, slice2, tree, proof));
      }
...
function OffchainState(config, options) {
  ...
  function OffchainStateInstance() {
    ...
    const merkleMaps = async () => {
      if (internal.merkleMap.root.toString() !== emptyMerkleMapRoot.toString() || internal.valueMap.size > 0) {
    ...
    async function get2(key, valueType) {
      let state3 = maybeContract().offchainStateCommitments.getAndRequireEquals();
    ...
    function field(index, type) {
      ...
      return {
        ...
        overwrite(value) {
          ...
          let update2 = getContract().self;
          update2.body.actions = Actions3.pushEvent(update2.body.actions, action);
        },
        update({ from, to }) {
          let action = toAction({
            prefix,
    ...
    return {
      setContractInstance(contractInstance) {
        ...
        internal._contractClass = contractInstance.constructor;
      },
      ...
      async settle(proof) {
        proof.verify();
        let state3 = getContract().offchainStateCommitments.getAndRequireEquals();
  ...
  const memoizedInstances = /* @__PURE__ */ new Map();
  return {
...
var BatchReducer = class {
  constructor({ actionType, batchSize, maxUpdatesPerProof = 300, maxUpdatesFinalProof = 100, maxActionsPerUpdate = Math.min(batchSize, 5) }) {
    ...
    assert2(maxActionsPerUpdate <= batchSize, "Invalid maxActionsPerUpdate, must be smaller than the batch size because we process entire updates at once.");
   ...
   */
  dispatch(action) {
    let update2 = this.contract().self;
    let canonical = Provable.toCanonical(this.actionType, this.actionType.fromValue(action));
    let fields2 = this.actionType.toFields(canonical);
    update2.body.actions = Actions3.pushEvent(update2.body.actions, fields2);
  }
  /**
  ...
  dispatchIf(condition, action) {
    ...
    let canonical = Provable.toCanonical(this.actionType, this.actionType.fromValue(action));
    let fields2 = this.actionType.toFields(canonical);
    let newActions = Actions3.pushEvent(update2.body.actions, fields2);
    update2.body.actions = Provable.if(condition, Actions3, newActions, update2.body.actions);
  }
  /**
